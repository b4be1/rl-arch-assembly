...
class AllPartMarkerReward(PartMarkerReward):
    """
    A concrete implementation of PartMarkerReward that takes all parts in the scene (placed parts, current part, and
    ...
    def __init__(self, max_marker_distance: Optional[float] = None,
                 ...
                 final_timestep_reward_scale: Optional[float] = 0.5, logarithmic_penalty_weight: float = 0.01,
        ...
        """
        super().__init__("all", max_marker_distance, intermediate_timestep_reward_scale,
                         final_timestep_reward_scale, abbreviated_name="part_pos",
                         logarithmic_penalty_weight=logarithmic_penalty_weight,
                         marker_pos_tolerance=marker_pos_tolerance)

    def _get_parts(self) -> Sequence[Part]:
        return self.task.placed_parts + [self.task.current_part] + self.task.future_parts
