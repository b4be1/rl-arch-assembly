...
class GripperTargetJointAccelerationReward(TargetJointAccelerationReward):
    """
    A reward that punishes high differences of the current joint velocities to the target joint velocities for the
    gripper.
    """

    def __init__(self, robot_name: str = "ur10", max_joint_velocity_difference: float = 2.0,
                 intermediate_timestep_reward_scale: float = 1e-4, final_timestep_reward_scale: Optional[float] = None,
                 exponent: float = 2.0):
        """
        ...
        :param exponent:                            the exponent to use in the calculation of the cost (e.g. 2 for a
                                                    squared cost)
        """
        super().__init__("gripper_", max_joint_velocity_difference, intermediate_timestep_reward_scale,
                         final_timestep_reward_scale, exponent)
        self.__robot_name = robot_name

    def _get_robot_component(self) -> RobotComponent:
        return self.task.robots[self.__robot_name].gripper
